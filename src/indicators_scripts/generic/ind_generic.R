...
# continent <- "Africa"
# input  <- "//dapadfs/workspace_cluster_13/GATES/rds/chirps/"
...
ind_precipitation <- function(root, base, continent, input, output,year,type,coress){
  ...
  system.time(ind_prec_cal<- foreach(i=1:nrow(prec), .verbose = TRUE) %dopar% {  #nrow(prec)
    require(dplyr)
    mylist <- list()
...
# root <- "//dapadfs/Workspace_cluster_2/SubsettingTool/"
# base <- raster(paste0(root,"Inputs/raster_base.asc"))
# input  <- "//dapadfs/workspace_cluster_13/GATES/rds/chirts/"
# output <- paste0(root,"output/")
# continent <- "Europe"
# year <- 1983
# type <- "ind_temp"
...
#Proceso en paralelo 
  ...
  system.time(index_temp <- foreach(i=1:nrow(tmin), .verbose = TRUE) %dopar% {  
    require(dplyr)
    mylist <- list()
...
long_day <- function(root, year,input,continent,output,type,coress,base){
  ...
  date <- seq(as.Date(paste0(year,"/1/1")), by = "month", length.out = 12)
    #Proceso en paralelo 
  library(doSNOW);library(foreach);library(parallel);library(doParallel)
  cores<- detectCores();cl<- makeCluster(coress); registerDoParallel(cl)
  long <- foreach(i=1:nrow(cor) ) %dopar% { #
    require(geosphere)
    require(dplyr)
    mylist <- list()
...
vpd <- function(year,root,input,output,continent,type,coress){
  ...
  system.time(calc_vpd <- foreach(i=1:nrow(tmin), .verbose = TRUE) %dopar% { 
    require(dplyr)
    mylist <- list()
    time.serie <- tmax[i, 1:ncol(tmax)]
    time.serie.1 <- tmin[i, 1:ncol(tmin)]

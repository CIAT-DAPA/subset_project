...
def calculate_summary(data, vars):
    ...
    for var in vars:
        vals = [data[var].apply(lambda x: float(x)).mean(), 
                data[var].apply(lambda x: float(x)).quantile(0.25), 
                data[var].apply(lambda x: float(x)).quantile(0.5), 
                data[var].apply(lambda x: float(x)).quantile(0.75),
                data[var].apply(lambda x: float(x)).std()]
...
def merge_data(custom_data, id_custom_data, id_accessions):
    id_list = custom_data[id_custom_data].apply(lambda x: int(x)).tolist()
